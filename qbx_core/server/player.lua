...
function CheckPlayerData(source, playerData)
    ...
    playerData.metadata.has_identity = playerData.has_identity or false -- <== ADICIONE ESTA LINHA EXATAMENTE AQUI
...
function AddMoney(identifier, moneyType, amount, reason)
    ...
    if amount < 0 then return false end -- Mantive a checagem original, mas a do 'not player.PlayerData.money[moneyType]' foi movida para depois da interceptação.

    local resourceName = GetInvokingResource()

    -- // INÍCIO DA MODIFICAÇÃO - SPACE ECONOMY // --
    -- Se o script que está tentando dar dinheiro NÃO FOR o 'space_economy',
    -- ele deve solicitar o pagamento ao cofre central.
    if resourceName ~= 'space_economy' then
        local success = exports['space_economy']:RequestPaymentFromVault(player.PlayerData.source, moneyType, amount, reason)
        return success
    end
    -- \\ FIM DA MODIFICAÇÃO // --

    -- O código abaixo só será executado se a chamada vier do 'space_economy',
    -- permitindo que o pagamento do cofre seja efetuado.
    if not player.PlayerData.money[moneyType] then
        -- Se o tipo de dinheiro não existir na conta do jogador, ele é criado aqui.
...
-- Atualiza o saldo no inventário
